...
| 2026-09-10 | **🔥 ROOT CAUSE "absensi private tidak muncul" (v2.3)**: Saat add billing cycle, bagian tabel di HTML ubah menjadi wrapper `<div id="rk-wrap-absensi">` **tanpa `<table>`**, sedangkan `renderAbsensiWorksheet()`/`renderMateriTable()` masih manggil `getElementById('rk-table-absensi'/'rk-table-materi')` → **null → `TypeError` di runtime** → tabel tidak pernah render (error hanya di console, tanpa alert). Fix: restore `<table class="rk-table" id="rk-table-absensi">` / `rk-table-materi` di dalam wrapper div. `APP_VERSION` → 8.0. | `rekap-absensi-module.js:208-218`, `assets/js/index.js:29` |
...
| 2026-09-10 | **v2.4 — Alokasi siklus private mengikuti model Billing global grup** (belajar dari code Billing module user): unit = `pertemuan_private.id` (bobot `jumlah_sesi`); sesi dikonsumsi **kronologis GABUNGAN semua kelas** dalam group; **Regula** — periode lebih old wajib pieni sampai kuota (jumlah sesi) keshanyan sebelum periode lebih baru; sesi sebelum awal periode pertama diskip; pertemuan 2-sesi memotong batas → **split/carry**; statistik `pakai/sisa/habis` + overflow di siklus terbaru → tampil di label dropdown & header grup (`Siklus N (4 sesi) · Sisa kuota 2`); note "Kuota global gabungan semua kelas group". `APP_VERSION` → 8.1. | `rekap-absensi-module.js` (`buildGlobalPeriodAllocation`, `buildPeriodMap`, `periodLabel`), `assets/js/index.js:29` |
